...
const SEASONS = [
  { key: 'jaro', name: 'Jaro', grass: ['#62b252', '#8ad06a', '#4c9444'], flowers: ['#ff9ec0', '#ffffff', '#ffd23f', '#b89ae8'],
    trees: [['#f7a8c4', '#fdd6e4'], ['#7cc86a', '#a8e08a'], ['#6cbc5e', '#94d676']], pine: '#3f7a4a', amb: 'petals', rain: 0.3, music: 2, sprite: 'tulip' },
  { key: 'leto', name: 'Léto', grass: ['#4f9e42', '#74c25a', '#3c8236'], flowers: ['#ffd23f', '#ffffff', '#e8484e', '#6ab4f0'],
    trees: [['#2f7a3a', '#4a9a48'], ['#3a8a40', '#5aaa52'], ['#27693a', '#3f8a44']], pine: '#2a6a3a', amb: 'motes', rain: 0.15, music: 3, sprite: 'sun' },
  { key: 'podzim', name: 'Podzim', grass: ['#9a9a48', '#c0b458', '#7a7836'], flowers: ['#e8843a', '#d65a31', '#f2b441'],
    trees: [['#e8843a', '#f6a55a'], ['#d65a31', '#ea7a4a'], ['#f2b441', '#f8d070'], ['#c8482e', '#e0643e']], pine: '#4a5a3a', amb: 'leaves', rain: 0.35, music: 0, sprite: 'acorn' },
  { key: 'zima', name: 'Zima', grass: ['#e4ebf6', '#ffffff', '#c4d0e6'], flowers: null,
    trees: [['#e8eef8', '#ffffff']], pine: '#2e5a52', amb: 'snow', rain: 0, music: 1, sprite: 'snow' }
];
...
const FOODS = ['susenky', 'ryby', 'chleb', 'mleko'];
...
const B = {
  ...
  drevorubec: { n: 'Dřevorubecká chata', cat: 'vyroba', w: 2, h: 2, cost: 40, wood: 0, work: 6, workers: 1, chop: 5, chimney: [7, 0], desc: 'Kácí dospělé stromy v okruhu 5. Pařezy znovu dorostou.' },
  ...
  pole: { n: 'Políčko', cat: 'priroda', w: 2, h: 2, cost: 20, wood: 2, work: 4, field: 1, desc: 'Kočky sázejí a sklízejí. Plodinu vybereš.' },
  ...
  molo: { n: 'Rybářské molo', cat: 'priroda', w: 1, h: 1, cost: 50, wood: 5, work: 5, workers: 1, water: 1, fish: 9, desc: 'Staví se na vodu u břehu. Rybář chytá ryby.' },
  ...
  spizirna: { n: 'Spižírna', cat: 'domov', w: 2, h: 2, cost: 60, wood: 8, work: 8, store: 50, desc: 'Společný sklad (+50 místa). Kočky si tu berou jídlo.' },
  nastenka: { n: 'Nástěnka', cat: 'domov', w: 1, h: 1, cost: 0, fixed: 1, board: 1, desc: 'Zakázky od sousedů.' },
  kvetiny: { n: 'Záhon květin', cat: 'ozdoby', w: 1, h: 1, cost: 8, instant: 1, cozy: 1, flower: 1, nodoor: 1, desc: '+1 útulnost. Včely je milují.' },
  ...
  lavka: { n: 'Lávka', cat: 'cesty', w: 1, h: 1, cost: 15, wood: 2, ground: 'bridge', zone: 'potok', desc: 'Most přes vodu.' },
  zbourat: { n: 'Zbourat', cat: 'cesty', tool: 1, desc: 'Vrátí polovinu ceny. Strom dá 1 dřevo.' }
};
const OUTCAP = 6;
...
/* ============ zones ============ */
const ZONES = {
  louka: { n: 'Louka', x0: 7, y0: 3, x1: 22, y1: 14, cost: 0, sign: null },
  potok: { n: 'Potok', x0: 23, y0: 0, x1: 29, y1: 17, cost: 200, sign: [23, 9], desc: 'Řeka s rybami. Molo a lávky.' },
  sad: { n: 'Starý sad', x0: 0, y0: 0, x1: 6, y1: 17, cost: 600, sign: [6, 8], desc: 'Divoké jabloně, které už plodí.' },
  sever: { n: 'Mýtina', x0: 7, y0: 0, x1: 22, y1: 2, cost: 900, sign: [14, 2], desc: 'Místo na další stavby.' },
  jih: { n: 'Jižní louka', x0: 7, y0: 15, x1: 22, y1: 17, cost: 1200, sign: [14, 15], desc: 'Místo na další stavby.' }
};

/* ============ festivals ============ */
const FEST = [
  { n: 'Jarní slavnost', need: { chleb: 8, mrkev: 12 }, coins: 300, first: ['zone', 'potok'], firstTxt: 'zóna Potok' },
  { n: 'Letní pouť', need: { jahody: 10, dzem: 4, mleko: 6 }, coins: 500, first: ['cat', 5], firstTxt: 'zlatá kočka Zlatíčko' },
  { n: 'Dožínky', need: { mouka: 16, dyne: 8, dynovy_kolac: 3 }, coins: 800, first: ['zone', 'sad'], firstTxt: 'zóna Starý sad' },
...
/* ============ tutorial quests (from Babička Ježková) ============ */
const QUESTS = [
  { t: 'Postav Mlýn — blízko políčka', ok: () => countB('mlyn') > 0, coins: 60 },
  { t: 'Postav Pekárnu — chléb je i kočičí jídlo', ok: () => countB('pekarna') > 0, coins: 60 },
  { t: 'Upeč 3 chleby', ok: () => (G.stats.made.chleb || 0) >= 3, coins: 50 },
  { t: 'Splň zakázku na nástěnce (záložka Zakázky)', ok: () => G.stats.orders > 0, coins: 60 },
  { t: 'Postav Dřevorubeckou chatu u stromů a přiřaď jí kočku', ok: () => allB().some(b => b.type === 'drevorubec' && b.built && b.workers.length), coins: 40, wood: 10 },
  { t: 'Postav třetí Kočičí domek', ok: () => countB('domek') >= 3, coins: 50 },
  { t: 'Zvyš útulnost na 9 (květiny, stromky, domky)', ok: () => coziness() >= 9, coins: 60 },
  { t: 'Přivítej čtvrtou kočičku (přijde ráno)', ok: () => G.cats.length >= 4, coins: 80 },
  { t: 'Postav Tržní stánek a označ zboží k prodeji', ok: () => countB('trziste') > 0, coins: 60 },
  { t: 'Získej ♥2 u některého souseda', ok: () => Object.keys(NEIGH).some(k => nbLevel(k) >= 2), coins: 100 },
  { t: 'Splň sezónní slavnost', ok: () => G.stats.fests > 0, coins: 150 }
];
